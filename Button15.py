...
img = (Image.open(r"Images/book_files/16.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: LGBT, Paranormal, Fantasy \nLanguage: English \nRating: M")
...
#Title of Book
title_1 = Label(win, text='Wolfsong', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
title_1.place(x=695, y=50)
...
text_2 = Label(win, text="Ox was twelve when his daddy taught him a very \nvaluable lesson. He said that Ox wasn’t worth \nanything and people would never understand him. \nThen he left. Ox was sixteen when he met the boy \non the road, the boy who talked and talked and talked. \nOx found out later the boy hadn’t spoken in almost \ntwo years before that day, and that the boy belonged \nto a family who had moved into the house at the end \nof the lane. Ox was seventeen when he found out the \nboy’s secret, and it painted the world around him in \ncolors of red and orange and violet, of Alpha and Beta \nand Omega. Ox was twenty-three when murder came \nto town and tore a hole in his head and heart. The boy \nchased after the monster with revenge in his bloodred \neyes, leaving Ox behind to pick up the pieces. It’s \nbeen three years since that fateful day—and the boy \nis back. Except now he’s a man, and Ox can no \nlonger ignore the song that howls between them.  ", font=('arial', 10), bg='#FCEFD3')
text_2.place(x=620,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=435)

#1 Star
def rate_1():
  rating = "1"
  file = open("15_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=428)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=428)
...
rating = []
with open("15_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=540)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("15_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=540)

reviews = []
with open("15_reviews.txt") as file:
   for line in file:
