...
img = (Image.open(r"Images/book_files/12.png"))
...
#Title of Book
title_1 = Label(win, text='Twisted Hate', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
...
text_1 = Label(win, text="Ana Huang", font=('arial', 10), bg='#FCEFD3')
...
text_2 = Label(win, text="Outgoing and ambitious, Jules Ambrose is a \nformer party girl who’s focused on one thing: passing \nthe attorney’s bar exam. The last thing she needs is to \nget involved with a doctor…especially insufferable, \nhotshot Josh Chen. But the more she gets to know \nhim, the more she realizes there’s more than meets \nthe eye to the man she’s hated for so long. Her best \nfriend’s brother. Her nemesis. And her only salvation. \nTheirs is a match made in hell, and when the \ndemons from their past catch up with them, they’re \nfaced with truths that could either save them …or \ndestroy everything they’ve worked for.", font=('arial', 10), bg='#FCEFD3')
text_2.place(x=620,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=340)

#1 Star
def rate_1():
  rating = "1"
  file = open("11_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=333)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=333)
...
rating = []
with open("11_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=445)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("11_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=445)

reviews = []
with open("10_reviews.txt") as file:
   for line in file:
