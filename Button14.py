...
img = (Image.open(r"Images/book_files/15.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: Fantasy, Romance, LGBT, Sexual \nLanguage: English \nRating: E")
...
#Title of Book
title_1 = Label(win, text='Captive Prince', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
...
text_1 = Label(win, text="C.S Pacat", font=('arial', 10), bg='#FCEFD3')
...
text_2 = Label(win, text="Damen is a warrior hero to his people, and the rightful \nheir to the throne of Akielos. But when his half brother \nseizes power, Damen is captured, stripped of his \nidentity, and sent to serve the prince of an enemy \nnation as a pleasure slave. Beautiful, manipulative, and \ndeadly, his new master, Prince Laurent, epitomizes \nthe worst of the court at Vere. But in the lethal political \nweb of the Veretian court, nothing is as it seems, and \nwhen Damen finds himself caught up in a play for the \nthrone, he must work together with Laurent to survive \nand save his country. For Damen, there is just one \nrule: never, ever reveal his true identity. Because the \none man Damen needs is the one man who has more \nreason to hate him than anyone else… ", font=('arial', 10), bg='#FCEFD3')
text_2.place(x=620,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=370)

#1 Star
def rate_1():
  rating = "1"
  file = open("14_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=363)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=363)
...
rating = []
with open("14_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=475)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("14_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=475)

reviews = []
with open("14_reviews.txt") as file:
   for line in file:
