...
img = (Image.open(r"Images/book_files/14.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: Romance, LGBT, Sexual \nLanguage: English \nRating: M")
...
#Title of Book
title_1 = Label(win, text='Red, White and Royal Blue', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
...
text_1 = Label(win, text="Casey McQuiston", font=('arial', 10), bg='#FCEFD3')
...
text_2 = Label(win, text="First Son Alex Claremont-Diaz is the closest thing to \na prince this side of the Atlantic. With his intrepid \nsister and the Veep’s genius granddaughter, they’re \nthe White House Trio, a beautiful millennial marketing \nstrategy for his mother, President Ellen Claremont. \nInternational socialite duties do have \ndownsides—namely, when photos of a confrontation \nwith his longtime nemesis Prince Henry at a royal \nwedding leak to the tabloids and threaten \nAmerican/British relations. The plan for damage \ncontrol: staging a fake friendship between the First \nSon and the Prince. As President Claremont kicks off \nher reelection bid, Alex finds himself hurtling into a \nsecret relationship with Henry that could derail the \ncampaign and upend two nations. What is worth the \nsacrifice? How do you do all the good you can do? \nAnd, most importantly, how will history remember you?", font=('arial', 10), bg='#FCEFD3')
text_2.place(x=620,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=415)

#1 Star
def rate_1():
  rating = "1"
  file = open("13_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=408)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=408)
...
rating = []
with open("13_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=520)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("13_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=520)

reviews = []
with open("13_reviews.txt") as file:
   for line in file:
