...
img = (Image.open(r"Images/book_files/19.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: Adventure, Fantasy, Science Fiction \nLanguage: English \nRating: M")
...
#Title of Book
title_1 = Label(win, text='Kenobi', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
title_1.place(x=715, y=50)

#Author
label_1 = Label(win, text='Author:', font=('arial', 10, 'bold'), bg='#FCEFD3')
label_1.place(x=580,y=100)

text_1 = Label(win, text="John Jackson Miller", font=('arial', 10), bg='#FCEFD3')
...
text_2 = Label(win, text="The greatest hero in the galaxy faces his toughest \nchallenge yet: He must stop being Obi-Wan-and become \nCrazy Old Ben. For fans of movie-related novels like \nDarth Plagueis! In this original novel set between the \nevents of Star Wars: Episode III Revenge of the Sith and \nEpisode IV A New Hope, Obi-Wan Kenobi comes to the \naid of the residents of Tatooine during his exile. But he \nstruggles with his new mission when he realizes that \nprotecting Luke Skywalker-the last hope of the \ngalaxy-means setting aside his compassion and his \nJedi warrior training, for the future of the galaxy lies not \nwith Obi-Wan Kenobi, but with a mystical desert recluse \nknown only by the name of Crazy Old Ben.", font=('arial', 9), bg='#FCEFD3')
text_2.place(x=628,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=335)

#1 Star
def rate_1():
  rating = "1"
  file = open("18_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=328)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=328)
...
rating = []
with open("18_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=440)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("18_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=440)

reviews = []
with open("18_reviews.txt") as file:
   for line in file:
