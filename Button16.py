...
img = (Image.open(r"Images/book_files/17.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: Fantasy, Fiction, LGBT \nLanguage: English \nRating: M")
...
#Title of Book
title_1 = Label(win, text='The House in the Cerulean Sea', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
title_1.place(x=530, y=50)

#Author
label_1 = Label(win, text='Author:', font=('arial', 10, 'bold'), bg='#FCEFD3')
label_1.place(x=580,y=100)

text_1 = Label(win, text="TJ Klune", font=('arial', 10), bg='#FCEFD3')
...
text_2 = Label(win, text="A magical island. A dangerous task. A burning secret. \nLinus Baker leads a quiet, solitary life. At forty, he lives \nin a tiny house with a devious cat and his old records. \nAs a Case Worker at the Department in Charge Of \nMagical Youth, he spends his days overseeing the \nwell-being of children in government-sanctioned \norphanages. When Linus is unexpectedly summoned \nby Extremely Upper Management he's given a curious \nand highly classified assignment: travel to Marsyas \nIsland Orphanage, where six dangerous children reside: \na gnome, a sprite, a wyvern, an unidentifiable green \nblob, a were-Pomeranian, and the Antichrist. Linus \nmust set aside his fears and determine whether or not \nthey’re likely to bring about the end of days. But the \nchildren aren’t the only secret the island keeps. Their \ncaretaker is the charming and enigmatic Arthur \nParnassus, who will do anything to keep his wards \nsafe. As Arthur and Linus grow closer, long-held \nsecrets are exposed, and Linus must make a choice: \ndestroy a home or watch the world burn.", font=('arial', 9), bg='#FCEFD3')
text_2.place(x=628,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=440)

#1 Star
def rate_1():
  rating = "1"
  file = open("16_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=433)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=433)
...
rating = []
with open("16_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=545)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("16_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=545)

reviews = []
with open("16_reviews.txt") as file:
   for line in file:
