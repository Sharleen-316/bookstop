...
img = (Image.open(r"Images/book_files/3.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: LGBT, Romance, Graphic Novels \nLanguage: English \nRating: T")
...
#Title of Book
title_1 = Label(win, text='Heartstopper', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
...
text_1 = Label(win, text="Alice Oseman", font=('arial', 10), bg='#FCEFD3')
text_1.place(x=635, y=100)

#Blurb
label_2 = Label(win, text="Blurb:", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_2.place(x=580,y=125)

text_2 = Label(win, text="Charlie, a highly-strung, openly gay over-thinker, and \nNick, a cheerful, soft-hearted rugby player, meet at a \nBritish all-boys grammar school. Friendship blooms \nquickly, but could there be something more...? Charlie \nSpring is in Year 10 at Truham Grammar School for \nBoys. The past year hasn't been too great, but at least \nhe's not being bullied anymore. Nick Nelson is in Year \n11 and on the school rugby team. He's heard a little \nabout Charlie - the kid who was outed last year and \nbullied for a few months - but he's never had the \nopportunity to talk to him. They quickly become friends, \nand soon Charlie is falling hard for Nick, even though \nhe doesn't think he has a chance. But love works in \nsurprising ways, and sometimes good things are \nwaiting just around the corner...", font=('arial', 10), bg='#FCEFD3')
text_2.place(x=620,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=390)

#1 Star
def rate_1():
  rating = "1"
  file = open("2_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=383)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=383)
...
rating = []
with open("2_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=495)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("2_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=495)

reviews = []
with open("2_reviews.txt") as file:
   for line in file:
