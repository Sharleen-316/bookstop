...
img = (Image.open(r"Images/book_files/18.png"))
...
def message():
  messagebox.showinfo("Content Tags/Ratings/Language", "Content Tags: Science, Nonfiction \nLanguage: English \nRating: G")
...
#Title of Book
title_1 = Label(win, text='Vital Science', font=('lucida bright', 20, 'bold'), bg='#B89B79', fg='#584A38')
title_1.place(x=690, y=50)

#Author
label_1 = Label(win, text='Author:', font=('arial', 10, 'bold'), bg='#FCEFD3')
label_1.place(x=580,y=100)

text_1 = Label(win, text="Dr Karl Kruzelnicki", font=('arial', 10), bg='#FCEFD3')
...
text_2 = Label(win, text="Get your pulse racing with Australia's most popular \nscientist, Dr Karl Kruszelnicki. Discover why people tell \nlies, why some planets are hotter than stars and how \nhumans are terraforming the Earth. Is cockroach milk \nreally the next Superfood? \nWhy are birds so smart? Why do trees need each other \nto grow and how do they communicate? Why did a group \nof scientists voluntarily starve to death while surrounded \nby tonnes of food? How long does a decapitated head \nstay 'alive'? What human artefacts can be seen with \nthe naked eye from the International Space Station? Who \nis Bertha Benz and how did her first car trip revolutionise \nhow we use petrol today? Finally, monitor your breathing \nand learn why whales are so big, why oral histories are \nsurprisingly accurate and try the five tried-and-tested \nsteps to becoming a wellness guru. ", font=('arial', 9), bg='#FCEFD3')
text_2.place(x=628,y=125)

#Rating Section
label_4 = Label(win, text="Your Rating: ", font=('arial', 10, 'bold'), bg='#FCEFD3')
label_4.place(x=580, y=385)

#1 Star
def rate_1():
  rating = "1"
  file = open("17_ratings.txt", "a")
  file.write(rating + '\n')
  file.close()
  messagebox.showinfo("Success!", "Thank you for your submission. You have rated 1 star.")
canvas = Canvas(win, width=35, height=35)
canvas.place(x=670, y=378)
...
Button(win, image=new_star, command=rate_1).place(x=670, y=378)
...
rating = []
with open("17_ratings.txt") as file:
   for line in file:
...
clear_button.place(x=708, y=490)

def post():
  review = text_3.get(1.0, 1.49)
  file = open("17_reviews.txt", "a")
  file.write(review + '\n')
  file.close()
...
post_button.place(x=670, y=490)

reviews = []
with open("17_reviews.txt") as file:
   for line in file:
